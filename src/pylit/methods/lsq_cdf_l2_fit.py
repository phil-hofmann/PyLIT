...
def _standard(D, E, lambd) -> Method:
    ...
    def f(alpha, R, F) -> FLOAT_DTYPE:
        ...
        return 0.5 * np.sum((R @ alpha - F) ** 2) + lambd * 0.5 * np.sum(np.cumsum((E_alpha - D)**2))
    ...
    def grad_f(alpha, R, F) -> ARRAY:
        ...
        return R.T @ (R @ alpha - F) + lambd * np.sum(np.dot(np.cumsum(E_alpha - D), np.tril(np.sum(E, axis=1))))
