...
# from pylit.backend.models.lrm import RegularLinearRegressionModel


# class LogisticRLRM(RegularLinearRegressionModel):
#     """This is the regular linear regression model with Logistic model functions.

#     TODO"""

#     def __init__(
#         self, omegas: ARRAY, s: ARRAY, beta: FLOAT_DTYPE = 1.0, order: str = "0,1"
#     ):
...
#             raise TypeError(f"The parameter 's' must be of type {ARRAY}.")
#         if np.any(s <= 0.0):
#             raise ValueError("The parameter 's' must be positive.")
#         # Type Conversion
#         omegas = omegas.astype(FLOAT_DTYPE)
#         s = s.astype(FLOAT_DTYPE)
#         beta = FLOAT_DTYPE(beta)
#         order = str(order)
#         # Initialize the Parent Class
#         if order == "0,1":
#             super().__init__(name="LaplaceRLRM", params=[omegas, s])
#         elif order == "1,0":
#             super().__init__(name="LaplaceRLRM", params=[s, omegas])
#         else:
...
#             reg_mat[:, i] = (
#                 np.exp(- tau * omega) * np.pi * s * (-tau) / np.sinh(np.pi * s * (-tau))
#             )
