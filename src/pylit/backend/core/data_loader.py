    ...
    def fetch(self) -> None:
        ...
        if self._file_path.endswith(".csv"):
            if self._header is None:
                self._data = np.genfromtxt(self._file_path, delimiter=",")
            else:
                self._data = np.genfromtxt(self._file_path, names=self._header, delimiter=",")
        else:
            if self._header is None:
                self._data = np.genfromtxt(self._file_path)
            else:
                self._data = np.genfromtxt(self._file_path, names=self._header)
